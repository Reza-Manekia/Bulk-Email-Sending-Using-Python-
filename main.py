...
class Email:
    ...
    def __init__(self, root):
        ...
        single = Radiobutton(root, text="Single", value="single", command=self.check_single_OR_bulk,
                             activebackground="skyblue", variable=self.var_choice, font=("times new roman", 18, "bold"),
                             bg="white", fg="black").place(x=250, y=120)
        multiple = Radiobutton(root, text="Multiple", value="multiple", command=self.check_single_OR_bulk,
                               ...
                               font=("times new roman", 18, "bold"), bg="white", fg="black").place(x=550, y=120)
        ...
        title = Label(self.root, text="Bulk Email Sender Panel",font=("Times New Roman", 48, "bold"), bg="black",
                      fg="white").place(x=0, y=0, relwidth=1)

        REF = Label(self.root, text="NOTE: Use Excel File For Sending Bulk Email At Once", font=("Calibri (body)", 13, "italic"),
                    bg="lightyellow", fg="black").place(x=0, y=80, relwidth=1)

        btn_set1 = Button(self.root, image=self.Email_icon,
                          bg="black", bd='0', command=LEFT, height=65, width=100,activebackground="black").place(x=1, y=4)

        btn_set = Button(self.root,image=self.Setting_icon,
                         bg="black", bd='0', command=self.setting_window,height= 65, width=100,activebackground="black",).place(x=890, y=4)

        #########################   text   #####################################

        To = Label(self.root, text="To: ", font=("times new roman", 15, "bold"), bg="white",
                   fg="black").place(x=50, y=200)
        Subject = Label(self.root, text="SUBJECT: ", font=("times new roman", 15, "bold"), bg="white",
                        fg="black").place(x=50, y=250)
        Message = Label(self.root, text="MESSAGE: ", font=("times new roman", 15, "bold"), bg="white",
                        fg="black").place(x=50, y=300)

        ############################################## STATUS ##################################################################

        self.Total = Label(self.root, font=("times new roman", 18, "bold"), bg="white", fg="black")
        self.Total.place(x=50, y=500)

        self.Sent = Label(self.root, font=("times new roman", 18, "bold"), bg="white", fg="darkgreen")
        self.Sent.place(x=350, y=500)

        self.Left = Label(self.root, font=("times new roman", 18, "bold"), bg="white", fg="orange")
        self.Left.place(x=450, y=500)

        self.Failed = Label(self.root, font=("times new roman", 18, "bold"), bg="white", fg="red")
        ...
        self.to_entry = Entry(self.root, font=("times new roman", 15), bg="lightgrey")
        self.to_entry.place(x=280, y=200, width=350, height=30)

        self.sub_entry = Entry(self.root, font=("times new roman", 15), bg="lightgrey")
        self.sub_entry.place(x=280, y=250, width=450, height=30)

        self.message_entry = Text(self.root, font=("times new roman", 15), bg="lightgrey")
        self.message_entry.place(x=280, y=300, width=700, height=190)

        btn1 = Button(root, activebackground="white", command=self.send_email, text="SEND",
                      font=("times new roman", 20, "bold"), bg="black",
                      fg="white").place(x=700, y=500, width=130, height=30)
        btn2 = Button(root, activebackground="white", command=self.clear1, text="CLEAR",
                      font=("times new roman", 20, "bold"), bg="#ffcccb",
                      fg="black").place(x=850, y=500, width=130, height=30)
        self.btn3 = Button(root, activebackground="skyblue", text="BROWSE", font=("times new roman", 15, "bold"),
    ...
    def send_email(self):
        ...
        if self.to_entry.get() == "" or self.sub_entry.get() == "" or x == 1:
            ...
            if self.var_choice.get() == "single":
                # For single email, use the name from the 'To' entry
                email = self.to_entry.get()
                name = self.extract_name_from_email(email)
                customized_message = f"Hello {name},\n\n" + self.message_entry.get('1.0', END)
                status = code_email.Email_send_function(email, self.sub_entry.get(), customized_message, self.uname, self.pasw)
                if status == "s":
            ...
            if self.var_choice.get() == "multiple":
                ...
                messagebox.showinfo("Success", "Emails have been sent. Please check the status.", parent=self.root)

    def extract_name_from_email(self, email):
        """Extract a name from the email address."""
        name_part = email.split('@')[0]
        name = ' '.join(name_part.split('.')).title()  # Convert john.doe to John Doe
        return name
    ...
    def setting_window(self):
        ...
        title2 = Label(self.root2, text="Bulk Email Sender", padx=10, compound=LEFT,
                       font=("Times New Roman", 48, "bold"), bg="black",
                       fg="white").place(x=0, y=0, relwidth=1)
        ...
        btn1 = Button(self.root2, activebackground="white", text="SEND", font=("times new roman", 13, "bold"),
                      bg="black",
                      fg="white", command=self.save_setting).place(x=250, y=250, width=100, height=20)
        btn2 = Button(self.root2, activebackground="white", text="CLEAR", font=("times new roman", 13, "bold"),
                      bg="#ffcccb", command=self.setting_clear,
                      fg="black").place(x=400, y=250, width=100, height=20)
